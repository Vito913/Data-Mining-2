...
best_tree_model = {"drop_percent": None, "part_of_speech": None, "best_alpha": None, "best_accuracy": None}
best_logistic_model = {"drop_percent": None, "part_of_speech": None, "best_lambda": None, "best_accuracy": None}
best_random_forest = {"drop_percent": None, "part_of_speech": None, "best_n_estimator": None, "best_accuracy": None, "best_max_features": None}
best_naive = {"drop_percent": None, "part_of_speech": None, "best_k": None, "best_accuracy": None}
...
for bigram in [False, True]:
    ...
    for pos in [False, True]:
        for drop_percent in [0.0 , 0.0012 , 0.0015 , 0.002 , 0.0025]:
            print("CURRENT PARAMETERS", bigram, pos, drop_percent)
            ...
            if not pos and bigram:
                current_matrix_train = doc_term_matrix_array_train_bigram
                feature_names_train  = vectorizedBigramTrain.get_feature_names_out()
            
            #drop columns based on drop percentage
            selected_columns = np.where(current_matrix_train.sum(axis=0) >= drop_percent * current_matrix_train.shape[1])[0]
            current_matrix_train = current_matrix_train[:, (current_matrix_train.sum(axis=0) >= drop_percent * current_matrix_train.shape[1])]
    ...
    print()
    print("ARE WE USING BIGRAMS?", bigram)
